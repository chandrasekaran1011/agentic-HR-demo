          ...
          {teams.map((t) => (
            <div key={t.id} className="rounded-lg border border-slate-800 bg-slate-900 p-5">
              ...
              <p className="text-sm text-slate-400 mt-2">
                Manager: <span className="text-slate-200">{t.manager}</span>
                <span className="text-slate-500 ml-2">{t.manager_email}</span>
              </p>
              <p className="text-sm text-slate-400 mt-1">
                Parking: <Badge className="ml-1" variant="outline">{t.parking_eligibility}</Badge>
              </p>
              <div className="mt-3 pt-3 border-t border-slate-800">
                <p className="text-xs text-slate-500 mb-2">Buddy pool ({t.buddy_pool.length})</p>
                <ul className="space-y-1">
                  {t.buddy_pool.map((b) => (
                    <li key={b.email} className="text-sm flex justify-between">
                      <span>
                        {b.name}
                        <span className="text-slate-500 ml-2">({b.role_family}, {b.tenure_years}yr)</span>
                      </span>
                    </li>
                  ))}
                </ul>
